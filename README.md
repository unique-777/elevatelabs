...
TASK 6:
In this task, I have  loaded the Iris dataset from a local CSV file, encoded the categorical target (species), and normalized the feature columns , then split the data into training and testing sets and trained a K-Nearest Neighbors (KNN) classifier while experimenting with different values of K. Finally,  evaluated the model using accuracy, confusion matrices, and a full classification report including precision, recall, and F1-scores. This provides a complete understanding of KNN performance on the dataset.
